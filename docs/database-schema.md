...
- `mst_categories` (หมวดหมู่สินค้า — `category_code`, `category_name`, `parent_id` nullable, `is_active` Soft Delete; Parent รหัส 1 ตัวอักษร / Child 2 ตัวอักษรตัวแรกสืบทอดจากแม่; จัดการผ่าน Master Data Hub `/settings/master-data?tab=categories`)
...
- `mst_sizes` (ไซส์มาตรฐาน Global Size รวมถึงไซส์บริการ — `size_code` สูงสุด 2 ตัวอักษร, `size_label`, `sort_order` Gap of 10, `is_active` Soft Delete; จัดการผ่าน Master Data Hub `/settings/master-data`)
...
- `product_models` (รุ่นสินค้า - Phase 1 ของการสร้าง Matrix — มี `is_service`, `is_raw_material`, `is_manufactured` Make vs Buy, `base_uom_id`, `purchasing_uom_id` หน่วยซื้อ bulk, `uom_conversion_factor` อัตราแปลง 1 หน่วยซื้อ → N หน่วยฐาน; Hard Delete รุ่นได้เฉพาะเมื่อ SKU ย่อยยังไม่ถูกใช้ใน `inventory_ledger` / `document_items` — ถ้ามีการเดินรายการแล้วให้ปิดใช้งาน `is_active` แทน)
...
- `production_job_operations` (ขั้นตอนการผลิตและค่าแรง In-house Routing / SAP Yield Confirmation — `job_id` → production_jobs, `operation_name`, `technician_id` → contacts, `confirmed_qty` ยอดยืนยัน, `unit_wage` ค่าแรงต่อหน่วยจาก Rate Card, `wage_cost` = confirmed_qty × unit_wage, `remark` เหตุผลเมื่อ override unit_wage, `technician_bill_id` โยง TB, `status` PENDING|COMPLETED — แยกจาก Service Assignment บน document_items)
...
- **Phase 19 VAT Ledger / Report Center:** รายงานภาษีขายดึงจาก `documents` ที่ `doc_type` IN (`TAX_INV`, `CS_TAX`, `ABB`) และรายงานภาษีซื้อจาก `AP_TAX` (สถานะ ISSUED/PAID) — ไม่มีตารางแยก ใช้ `contacts.tax_id` / `contacts.branch_code` และ `ecommerce_buyer_name` เมื่อเป็นลูกค้าแพลตฟอร์ม; ทะเบียนเอกสารเพิ่มเติม: `INV_DO` จาก `documents`, `EXP` จาก `expenses` (`expense_date`, `vendor_id` → `contacts`), `PAY` จาก `documents.doc_type = PAY` (ใบสำคัญจ่าย — `payment_transactions` เป็นรายการย่อยของเอกสาร PAY)
