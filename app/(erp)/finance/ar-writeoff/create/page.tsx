...
export const metadata: Metadata = {
  title: "เปิดบิลตัดหนี้สูญ | AR Write-off",
...
type ArWriteoffCreatePageProps = {
  searchParams: Promise<{ contact_id?: string }>;
};

export default async function ArWriteoffCreatePage({
  searchParams,
}: ArWriteoffCreatePageProps) {
    ...
    <div className="flex flex-col gap-6 p-6">
      <div className="flex flex-col gap-2">
        <h1 className="flex items-center gap-2 text-3xl font-bold tracking-tight text-slate-900">
          ...
          เปิดบิลตัดหนี้สูญ / ตัดเศษบัญชี
        </h1>
        <p className="text-slate-500">
          เลือกผู้ติดต่อจากตารางสรุปยอดหนี้ แล้วระบุยอดตัดหนี้ต่อบิล
          (INV_DO / TAX_INV / CS_TAX) · ใบวางบิล (BN) ใช้กรองรายการ · ไม่มีการรับเงินสด
        </p>
      </div>
